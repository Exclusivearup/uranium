...
const bookList= async function (req, res) {
    let allBooks= await BookModel.find().select({bookName:1,athorName:1,_id:0})

    res.send({msg:allBooks})
}
const getBooksInYear= async function (req,res) {
    // let yr= req.body.year
    let booksInYear =await BookModel.find({year:req.body.year}).select({bookName:1, _id:0})
...
//   let BookName = req.body.bookName
  let particularBooks = await BookModel.find({bookName:req.body.bookName , year:req.body.year}).select({bookName:1,authorName:1, _id:0})
//   let year=req.body.year
  // let particularBooks2 =await BookModel.find({year:req.body.year})
...
// let indPrice = req.body.indianPrice
    let INRBooks =await BookModel.find({indianPrice:{$in:["INR100", "INR200" ,"INR500"]}})
...
// BookModel.find( { $or: [  {strockAvailable: true}, {totalPages: {$gt: 500 }  ] })
